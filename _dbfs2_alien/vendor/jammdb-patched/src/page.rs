...
impl Page {
    ...
    pub(crate) fn from_buf(buf: &[u8], id: PageID, pagesize: u64) -> &Page {
        #[allow(clippy::cast_ptr_alignment)]
        unsafe {
            &*(buf.as_ptr().add((id * pagesize) as usize) as *const Page)
        }
    }
    #[inline]
    ...
    pub(crate) fn meta(&self) -> &Meta {
        ...
        unsafe { &*(core::ptr::addr_of!(self.ptr) as *const Meta) }
    }

    pub(crate) fn meta_mut(&mut self) -> &mut Meta {
        assert_eq!(self.page_type, Page::TYPE_META);
        unsafe { &mut *(core::ptr::addr_of_mut!(self.ptr) as *mut Meta) }
    }

    pub(crate) fn freelist(&self) -> &[PageID] {
        assert_eq!(self.page_type, Page::TYPE_FREELIST);
        unsafe {
            let start = core::ptr::addr_of!(self.ptr) as *const PageID;
    ...
    pub(crate) fn freelist_mut(&mut self) -> &mut [PageID] {
        assert_eq!(self.page_type, Page::TYPE_FREELIST);
        unsafe {
            let start = core::ptr::addr_of!(self.ptr) as *mut PageID;
