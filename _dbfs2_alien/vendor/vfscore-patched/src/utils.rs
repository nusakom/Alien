...
bitflags::bitflags! {
    ...
    pub struct VfsNodePerm: u16 {
        ...
        ///
        /// 上游 `VfsNodePerm` 只定义了低 9 位，导致 `chmod 04755` 的 setuid 位在
        /// `VfsInodeMode -> VfsNodePerm` 这一步被 `from_bits_truncate` 静默丢弃。
        /// pjdfstest 的 `chmod/00.t`、`chown/00.t` 有数十条断言要求 `stat mode`
        /// 回读出 `06555`/`02755`/`01700`，全部因此失败。
        const SET_UID = 0o4000;
