...
impl VfsPath {
    ...
    /// 跟随符号链接 `symlink`：读出链接目标，拼成新路径后继续 open。
    /// `depth` 是进入本次跟随前已经跟随过的符号链接层数。
    fn to_symlink(
        &self,
        symlink: Arc<dyn VfsDentry>,
        depth: usize,
    ) -> VfsResult<Arc<dyn VfsDentry>> {
        let path = read_link_target(&symlink)?;
        if path.starts_with("/") {
            trace!("[to_symlink] absolute path: {}", path);
            // absolute path
            let new_path = Self::new(self.root.clone(), self.root.clone()).join(path)?;
            new_path.__open_depth(None, true, depth + 1)
        } else {
            trace!("[to_symlink] relative path: {}", path);
            // relative path
            let p = symlink.parent().ok_or(VfsError::NoEntry)?;
    ...
    /// 不跟随末端符号链接地打开路径（对应 `AT_SYMLINK_NOFOLLOW`）。
    ///
    /// 用于 `lchown` / `lchmod` / `lstat` 等语义：当路径最后一段是符号链接时，
    /// 直接打开该链接本身，而不是它指向的目标。等价于 `open2(mode, O_NOFOLLOW)`，
    /// 但不要求调用方构造 `OpenFlags`，从而避免与 `constants::io::OpenFlags` 的类型耦合
    /// （内核侧 `open2` 形参类型是 `pconst::io::OpenFlags`）。
    ///
    pub fn open_nofollow(&self, mode: Option<VfsInodeMode>) -> VfsResult<Arc<dyn VfsDentry>> {
        self.__open(mode, false)
    }

    fn __open(&self, mode: Option<VfsInodeMode>, symlink: bool) -> VfsResult<Arc<dyn VfsDentry>> {
        self.__open_depth(mode, symlink, 0)
    }

    /// `__open` 的带深度版本。`depth` 是本次解析已经跟随过的符号链接层数。
    fn __open_depth(
        &self,
        mode: Option<VfsInodeMode>,
        symlink: bool,
        ...
        let exist = self.__exists_depth(depth);
        match exist {
            Ok(d) => {
                if symlink {
                    let inode = d.inode()?;
                    match inode.inode_type() {
                        VfsNodeType::SymLink if depth >= MAXSYMLINKS => Err(VfsError::Loop),
                        VfsNodeType::SymLink => self.to_symlink(d, depth),
                        _ => Ok(d),
                    }
                } else {
                    Ok(d)
    ...
    pub fn exists(&self) -> VfsResult<Arc<dyn VfsDentry>> {
        self.__exists_depth(0)
    }

    /// `exists` 的带深度版本。`depth` 是本次解析已经跟随过的符号链接层数。
    fn __exists_depth(&self, depth: usize) -> VfsResult<Arc<dyn VfsDentry>> {
        ...
        loop {
            ...
            match name {
                ...
                _ => {
                    ...
                    let sub_dentry = if sub_dentry.is_none() {
                        // second, we find in inode cache or disk
                        let parent_inode = dentry.inode()?;
                        let sub_inode = parent_inode.lookup(name)?;
                        // if we find the inode, we insert it into dentry cache
                        dentry.insert(name, sub_inode)?
                    } else {
                        sub_dentry.unwrap()
                    };
                    if rest.is_some()
                        && sub_dentry.inode()?.inode_type() == VfsNodeType::SymLink
                    {
                        if depth >= MAXSYMLINKS {
                            return Err(VfsError::Loop);
                        }
                        let target = read_link_target(&sub_dentry)?;
                        let new_fs = if target.starts_with('/') {
                            self.root.clone()
                        } else {
                            // 相对符号链接：相对它**所在的目录**解析
                            parent.clone()
                        ...
                        let mut joined = String::with_capacity(target.len() + rest.len() + 1);
                        joined.push_str(&target);
                        joined.push('/');
                        joined.push_str(rest);
                        let new_path = VfsPath {
                            root: self.root.clone(),
                            fs: new_fs,
                            path: joined,
                        };
                        return new_path.__exists_depth(depth + 1);
                    }
                    parent = sub_dentry;
                }
            }
    ...
    pub fn symlink(&self, target: &str) -> VfsResult<()> {
        let this = self.open_nofollow(None);
        match this {
    ...
    pub fn rename_to(
        ...
        let old_last = self.filename();
        let new_last = new_vfs_path.filename();
        if old_last == "." || old_last == ".." || new_last == "." || new_last == ".." {
            return Err(VfsError::EBUSY);
        }
        let old_dt = self.open_nofollow(None)?;
        checkout_busy(&old_dt, &context)?;
        let new_dt = new_vfs_path.open_nofollow(None);
        if new_dt.is_err() {
            ...
            if old_dt.inode()?.inode_type() == VfsNodeType::Dir
                && new_dt.inode()?.inode_type() != VfsNodeType::Dir
            {
                return Err(VfsError::NotDir);
            }
            let new_parent = new_vfs_path
...
/// 读出符号链接 `symlink` 指向的目标路径。
///
/// 抽成函数是因为 `__exists_depth`（跟随中间分量）和 `to_symlink`（跟随末级分量）
/// 都要做同一件事，且两者的缓冲区大小与深度上限必须一致。
fn read_link_target(symlink: &Arc<dyn VfsDentry>) -> VfsResult<String> {
...
impl Iterator for DirIterImpl {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.inode.readdir(self.index) {
            Ok(Some(x)) => {
                self.index += 1;
                Some(x)
            }
