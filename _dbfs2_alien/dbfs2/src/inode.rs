...
/// D2b: 从已持久化的 DBFS inode bucket 重建 inode 分配器的**下一可用号**。
///
/// **不新增任何持久化字段**：inode 号本身就是持久化状态 —— 每个 inode 就是一个
/// 名为 `ino.to_be_bytes()` 的**根级** bucket。这里只是把它读回来。
///
/// 语义 = `max(当前存活 inode 号) + 1`，即 **next inode number**，而不是
/// "最小可用 inode"：已删除 inode 留下的中间空洞**不会**被复用（例如存活的
/// `1,2,4,5` ⇒ `max=5` ⇒ `next=6`）。这保证不与当前存活 inode 冲突；空洞复用
/// 不是本函数的目标，也是设计上刻意允许的行为。
///
/// 空库（无任何 inode bucket）返回 `1`，以配合 `dbfs_common_root_inode` 首次
/// 创建 root 时的 `assert_eq!(old, 1)`（见 `fs_type.rs`）。仅含 root(=1) 的库返回 `2`。
///
/// 判别式（必须双条件，不能对 bucket 名直接做整数转换）：
///   1. `name.len() == size_of::<usize>()`（8 字节）—— 排除 `super_blk`(9B)、`tx_demo`(7B)；
///   2. 该 bucket 含 KV `"mode"` —— 每个 inode bucket 必然有（root: `fs_type.rs`，
///      create: `dbfs_rvfs_create`），排除未来可能出现的长同名非 inode bucket。
///
/// 复杂度：只扫**根级** bucket，不递归、不读取任何 KV 值 ⇒ O(N log N) 纯 CPU，
/// 且页访问走内存镜像（`IndexByPageIDImpl::index`），**不产生任何设备 I/O**。
pub(crate) fn dbfs_recover_inode_number(db: &jammdb::DB) -> DbfsResult<usize> {
    let tx = db.tx(false).map_err(DbfsError::from)?; // 只读事务，零写
    let mut max_ino = 0usize;
    for (name, bucket) in tx.buckets() {
        let raw = name.name();
        if raw.len() != core::mem::size_of::<usize>() {
            continue; // super_blk / tx_demo 等非 inode bucket
        }
        if bucket.get_kv("mode").is_none() {
            continue; // 二次判别：真 inode bucket 必然含 "mode"
        }
        // inode bucket 名是 `ino.to_be_bytes()` ⇒ 恢复必须用 from_be_bytes。
        let ino = usize::from_be_bytes(raw.try_into().unwrap());
        if ino > max_ino {
            max_ino = ino;
        }
    }
    Ok(if max_ino == 0 { 1 } else { max_ino + 1 })
}
